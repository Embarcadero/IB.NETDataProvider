...
namespace EntityFramework.InterBase.SqlGen
{
	internal class ExpressionTranslator : DbExpressionVisitor
	{
		...
		internal IBParameter CreateParameter(object value, TypeUsage type)
		{
			var parameterName = string.Concat("@p", _parameterNameCount.ToString(CultureInfo.InvariantCulture));
			_parameterNameCount++;

			var parameter = IBProviderServices.CreateSqlParameter(parameterName, type, ParameterMode.In, value);
