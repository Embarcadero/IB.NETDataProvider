...
namespace EntityFramework.InterBase.SqlGen
{
	internal sealed class SqlGenerator : DbExpressionVisitor<ISqlFragment>
	{
		...
		private static Dictionary<string, FunctionHandler> InitializeCanonicalFunctionHandlers()
		{
			...
			functionHandlers.Add("AddMilliseconds", (sqlgen, e) => HandleCanonicalFunctionDateTimeAdd(sqlgen, e, null)); // not supported
			...
			functionHandlers.Add("DiffMilliseconds", (sqlgen, e) => HandleCanonicalFunctionDateTimeDiff(sqlgen, e, null)); // not supported
		...
		public override ISqlFragment Visit(DbApplyExpression e)
		{
			...
			throw new NotSupportedException($"{joinString} statement is not supported in InterBase.");
				...
				switch (typeKind)
				{
					...
					case PrimitiveTypeKind.String:
						...
						var length = MetadataHelpers.GetFacetValueOrDefault<int?>(e.ResultType, MetadataHelpers.MaxLengthFacetName, null)
							?? (isUnicode ? IBProviderManifest.UnicodeVarcharMaxSize : IBProviderManifest.AsciiVarcharMaxSize);
		...
		{
			throw new NotSupportedException("The EXCEPT statement is not supported in InterBase.");
		...
		{
			throw new NotSupportedException("The INTERSECT statement is not supported in InterBase.");
		...
		private static ISqlFragment HandleCanonicalEndsWithFunction(SqlGenerator sqlgen, DbFunctionExpression e)
		{
			sqlgen._shouldHandleBoolComparison = false;
			var result = new SqlBuilder();
			result.Append("EF_REVERSE(");
			result.Append(e.Arguments[0].Accept(sqlgen));
			result.Append(") STARTING WITH EF_REVERSE(");
		...
		{
			return sqlgen.HandleFunctionDefaultGivenName(e, "EF_POSITION");
		}

		private static ISqlFragment HandleCanonicalFunctionLength(SqlGenerator sqlgen, DbFunctionExpression e)
		{
			return sqlgen.HandleFunctionDefaultGivenName(e, "EF_LENGTH");
		}
		...
		private static ISqlFragment HandleCanonicalFunctionSubstring(SqlGenerator sqlgen, DbFunctionExpression e)
		{
			...
			result.Append("EF_SUBSTR(");

			Debug.Assert(e.Arguments.Count == 3, "Substr should have three arguments from the IB UDF");
			result.Append(e.Arguments[0].Accept(sqlgen));
			result.Append(", ");
			result.Append(e.Arguments[1].Accept(sqlgen));
			result.Append(", ");
		...
		{
			return sqlgen.HandleFunctionDefaultGivenName(e, "EF_LOWER");
		}
		...
		{
			throw new NotSupportedException("CreateDateTimeOffset is not supported by InterBase.");
		}
		...
		{
			return sqlgen.HandleFunctionDefaultGivenName(e, "EF_NEWGUID");
		}
		#endregion
		...
		internal static string GetSqlPrimitiveType(TypeUsage type)
		{
			...
			switch (primitiveType.PrimitiveTypeKind)
			{
				...
				case PrimitiveTypeKind.String:
					...
					{
						if (length > (isUnicode ? IBProviderManifest.UnicodeVarcharMaxSize : IBProviderManifest.AsciiVarcharMaxSize))
						{
		...
		/// <summary>
		/// Decorate with double quotes and escape double quotes inside in InterBase.
		/// </summary>
