...
{
	public class IBMigrationSqlGenerator : MigrationSqlGenerator
	{
		readonly IIBMigrationSqlGeneratorBehavior _behavior;

		string _migrationsHistoryTableName;

		public IBMigrationSqlGenerator(IIBMigrationSqlGeneratorBehavior behavior = null)
		{
			_behavior = behavior ?? new DefaultIBMigrationSqlGeneratorBehavior();
		}
		...
		{
			throw new NotSupportedException("Moving procedure is not supported by InterBase.");
		}

		protected virtual IEnumerable<MigrationStatement> Generate(MoveTableOperation operation)
		{
			throw new NotSupportedException("Moving table is not supported by InterBase.");
		}
		...
		{
			throw new NotSupportedException("Renaming procedure is not supported by InterBase.");
		}
		...
		static string CheckName(string name)
		{
			...
			if (name.Length > LengthLimit)
				throw new ArgumentOutOfRangeException($"The name '{name}' is longer than InterBase's {LengthLimit} characters limit for object names.");
			return name;
		}
		...
		{
			return DbConfiguration.DependencyResolver.GetService<DbProviderFactory>(IBProviderServices.ProviderInvariantName).CreateConnection();
		}
