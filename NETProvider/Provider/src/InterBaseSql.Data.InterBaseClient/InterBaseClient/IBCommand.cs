...
public sealed class IBCommand : DbCommand, IIBPreparedCommand, IDescriptorFiller, ICloneable
{
	...
	public new Task<IBDataReader> ExecuteReaderAsync() => ExecuteReaderAsync(CommandBehavior.Default);
	public new Task<IBDataReader> ExecuteReaderAsync(CancellationToken cancellationToken) => ExecuteReaderAsync(CommandBehavior.Default, cancellationToken);
	...
	public new Task<IBDataReader> ExecuteReaderAsync(CommandBehavior behavior) => ExecuteReaderAsync(behavior, CancellationToken.None);
	public new async Task<IBDataReader> ExecuteReaderAsync(CommandBehavior behavior, CancellationToken cancellationToken)
	{
		CheckCommand();
	...
	internal async Task<DbValue[]> FetchAsync(CancellationToken cancellationToken = default)
	{
		if (_statement != null)
		{
			...
			{
				// Fetch the next row
				var values = await _statement.FetchAsync(cancellationToken).ConfigureAwait(false);
				if (values != null)
				{
					OnFetch(new FetchEventArgs(values));
				}
	...
	void IIBPreparedCommand.Release() => Release();
	...
	Task IIBPreparedCommand.ReleaseAsync(CancellationToken cancellationToken) => ReleaseAsync(cancellationToken);
	...
	void IIBPreparedCommand.TransactionCompleted() => TransactionCompleted();
	...
	Task IIBPreparedCommand.TransactionCompletedAsync(CancellationToken cancellationToken) => TransactionCompletedAsync(cancellationToken);
	...
	private void UpdateParameterValues(Descriptor descriptor)
	{
		...
		for (var i = 0; i < descriptor.Count; i++)
		{
			...
			if (_namedParameters.Count > 0)
			{
				...
				if (index == -1)
				{
					throw IBException.Create($"Must declare the variable '{_namedParameters[i]}'.");
				}
			}
	...
	private void ExecuteCommand(CommandBehavior behavior, bool returnsSet)
	{
			...
			behavior == CommandBehavior.Default)
		{
			...
			{
				throw IBException.Create("Must declare command parameters.");
			}

			// Execute
			_statement.Execute(this);
		}
	}
	private async Task ExecuteCommandAsync(CommandBehavior behavior, bool returnsSet, CancellationToken cancellationToken = default)
	{
			...
			behavior == CommandBehavior.Default)
		{
			...
			// Execute
			await _statement.ExecuteAsync(this, cancellationToken).ConfigureAwait(false);
		}
	}
