...
namespace InterBaseSql.Data.Isql
{
	class SqlStringParser
	{
		...
		public IEnumerable<IBStatement> Parse()
		{
			var lastYield = 0;
			var index = 0;
			...
			while (true)
			{
				...
				else
				{
					foreach (var token in Tokens)
					{
						...
						{
							index += token.Length;
							yield return new IBStatement(_source.Substring(lastYield, index - lastYield - token.Length), rawResult.ToString());
							lastYield = index;
							rawResult.Clear();
							goto Continue;
						}
			...
			if (index >= _sourceLength)
			{
                ...
                yield return new IBStatement(parsed, rawResult.ToString());
				lastYield = _sourceLength;
				rawResult.Clear();
			}
			else
			{
				yield return new IBStatement(_source.Substring(lastYield, index - lastYield), rawResult.ToString());
				lastYield = index;
				rawResult.Clear();
			}
		}
