...
#warning Who's disposing this?
				...
				catch (Exception ex)
				{
					...
					throw new IBException(string.Format("An exception was thrown when executing command: {1}.{0}Batch execution aborted.{0}The returned message was: {2}.",
							Environment.NewLine,
							statement.Text,
							ex.Message),
						ex);
		...
		protected void CreateDatabase(string createDatabaseStatement)
		{
			...
			IBConnection.CreateDatabase(_connectionString.ToString(), pageSize, true, false);
			_requiresNewConnection = true;
			ProvideConnection();
		}
		...
		protected IBCommand ProvideCommand()
		{
			if (_sqlCommand == null)
			{
		...
		protected IBConnection ProvideConnection()
		{
		...
		protected void OnCommandExecuting(IBCommand sqlCommand, SqlStatementType statementType)
		{
		...
		protected void OnCommandExecuted(IBDataReader dataReader, string commandText, SqlStatementType statementType, int rowsAffected)
		{
