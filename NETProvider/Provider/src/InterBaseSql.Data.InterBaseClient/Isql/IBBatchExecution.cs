...
public sealed class IBBatchExecution
{
	...
	IBStatementCollection _statements;
	IBConnection _sqlConnection;
	IBTransaction _sqlTransaction;
	IBConnectionStringBuilder _connectionString;
	IBCommand _sqlCommand;
	...
	/// </summary>
	public IBStatementCollection Statements
	{
		get { return _statements; }
	}

	/// <summary>
	/// Creates an instance of IBBatchExecution engine with the given
	/// connection.
	/// </summary>
	/// <param name="sqlConnection">A <see cref="IBConnection"/> object.</param>
	public IBBatchExecution(IBConnection sqlConnection = null)
	{
		_statements = new IBStatementCollection();
		if (sqlConnection == null)
		{
			_sqlConnection = new IBConnection(); // do not specify the connection string
			_connectionString = new IBConnectionStringBuilder();
		}
		else
		{
			_sqlConnection = sqlConnection;
			_connectionString = new IBConnectionStringBuilder(sqlConnection.ConnectionString);
		}
	}

	/// <summary>
	/// Appends SQL statements from <see cref="IBScript"/> instance. <see cref="IBScript.Parse"/> should be already called.
	/// </summary>
	/// <param name="isqlScript">A <see cref="IBScript"/> object.</param>
	public void AppendSqlStatements(IBScript isqlScript)
	{
	...
	public void Execute(bool autoCommit = true)
	{
		...
		foreach (var statement in Statements)
		{
				...
				statement.StatementType == SqlStatementType.SetSQLDialect))
			{
				...
				{
					_sqlTransaction = _sqlConnection.BeginTransaction(IBTransaction.DefaultIsolationLevel, null);
				}
			...
			try
			{
				switch (statement.StatementType)
				{
					...
					case SqlStatementType.DropDatabase:
						...
						IBConnection.DropDatabase(_connectionString.ToString());
					...
					case SqlStatementType.Disconnect:
						...
						IBConnection.ClearPool(_sqlConnection);
						_requiresNewConnection = false;
			...
			catch (Exception ex)
			{
				...
				var message = string.Format("An exception was thrown when executing command: {1}.{0}Batch execution aborted.{0}The returned message was: {2}.",
					...
					statement.Text,
					ex.Message);
				throw IBException.Create(message, ex);
			}
		}
	...
	public async Task ExecuteAsync(bool autoCommit = true, CancellationToken cancellationToken = default)
	{
		...
		foreach (var statement in Statements)
		{
				...
				statement.StatementType == SqlStatementType.SetSQLDialect))
			{
				...
				if (_sqlTransaction == null && !(statement.StatementType == SqlStatementType.Commit || statement.StatementType == SqlStatementType.Rollback))
				{
					_sqlTransaction = await _sqlConnection.BeginTransactionAsync(IBTransaction.DefaultIsolationLevel, null, cancellationToken).ConfigureAwait(false);
				}
				_sqlCommand.Transaction = _sqlTransaction;
			}
			...
			try
			{
				switch (statement.StatementType)
				{
					...
					case SqlStatementType.DropDatabase:
						...
						await IBConnection.DropDatabaseAsync(_connectionString.ToString(), cancellationToken).ConfigureAwait(false);
						_requiresNewConnection = true;
	...
	private void CreateDatabase(string createDatabaseStatement)
	{
		...
		IBConnection.CreateDatabase(_connectionString.ToString(), pageSize: pageSize);
		_requiresNewConnection = true;
		ProvideConnection();
	}
	/// <summary>
	...
	private async Task CreateDatabaseAsync(string createDatabaseStatement, CancellationToken cancellationToken = default)
	{
		...
		await IBConnection.CreateDatabaseAsync(_connectionString.ToString(), pageSize: pageSize, cancellationToken: cancellationToken).ConfigureAwait(false);
		_requiresNewConnection = true;
		await ProvideConnectionAsync(cancellationToken).ConfigureAwait(false);
	}
	...
	private IBCommand ProvideCommand()
	{
		if (_sqlCommand == null)
		{
			_sqlCommand = new IBCommand();
		}
	...
	private async Task<IBCommand> ProvideCommandAsync(CancellationToken cancellationToken = default)
	{
		if (_sqlCommand == null)
		{
			_sqlCommand = new IBCommand();
	...
	private IBConnection ProvideConnection()
	{
		if (_requiresNewConnection)
		{
			...
			_sqlConnection = new IBConnection(_connectionString.ToString());
		}
	...
	private async Task<IBConnection> ProvideConnectionAsync(CancellationToken cancellationToken = default)
	{
		if (_requiresNewConnection)
		{
			if (_sqlConnection != null && _sqlConnection.State != ConnectionState.Closed)
	...
	private void OnCommandExecuting(IBCommand sqlCommand, SqlStatementType statementType)
	{
	...
	/// </summary>
	/// <param name="commandText">The <see cref="IBCommand.CommandText"/> of the executed SQL command.</param>
	/// <param name="dataReader">The <see cref="IBDataReader"/> instance with the returned data. If the
	...
	private void OnCommandExecuted(IBDataReader dataReader, string commandText, SqlStatementType statementType, int rowsAffected)
	{
