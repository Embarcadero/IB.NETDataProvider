...
/// <summary>
/// IBScript parses a SQL file and returns its SQL statements.
/// The class take in consideration that the statement separator can change in code.
/// For instance, in InterBase databases the statement <c>SET TERM !! ;</c> will change the
...
public class IBScript
{
	...
	IBStatementCollection _results;

	/// <summary>
	/// Returns a IBStatementCollection containing all the SQL statements (without comments) present on the file.
	/// This property is loaded after the method call <see cref="Parse"/>.
	/// </summary>
	public IBStatementCollection Results
	{
		get { return _results; }
	}

	/// <summary>
	/// Creates IBScript reading content from file.
	/// </summary>
	public static IBScript LoadFromFile(string fileName)
	{
		return new IBScript(File.ReadAllText(fileName));
	}

	public IBScript(string script)
	{
		if (script == null)
			throw new ArgumentNullException();

		_results = new IBStatementCollection();
	...
	static SqlStatementType? GetStatementType(string sqlStatement)
	{
		switch (sqlStatement.FirstOrDefault())
		{
			...
			case 'a':
				...
				if (sqlStatement.StartsWith("alter generator", StringComparison.OrdinalIgnoreCase))
				{
