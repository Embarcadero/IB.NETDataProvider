...
internal sealed class IBArray : ArrayBase
{
	#region Fields
	...
	public override DatabaseBase Database
	{
		...
		set { _database = (IBDatabase)value; }
	}

	public override TransactionBase Transaction
	{
		get { return _transaction; }
		set { _transaction = (IBTransaction)value; }
	}
	...
	public IBArray(ArrayDesc descriptor)
		: base(descriptor)
	{
		_statusVector = new IntPtr[IscCodes.ISC_STATUS_LENGTH];
	}

	public IBArray(IBDatabase database, IBTransaction transaction, string tableName, string fieldName)
		: this(database, transaction, -1, tableName, fieldName)
	{ }

	public IBArray(IBDatabase database, IBTransaction transaction, long handle, string tableName,	string fieldName)
	...
	public override byte[] GetSlice(int sliceLength)
	{
		...
		if (_database.IBClient.IBClientVersion >= 7)
			arrayDesc = ArrayDescMarshaler.MarshalManagedToNative2(Descriptor);
		else
			arrayDesc = ArrayDescMarshaler.MarshalManagedToNative(Descriptor);

		var buffer = new byte[sliceLength];

		if (_database.IBClient.IBClientVersion >= 7)
		{
			_database.IBClient.isc_array_get_slice2(
			_statusVector,
			ref dbHandle,
			ref trHandle,
			ref _handle,
			...
			buffer,
			ref sliceLength);
			ArrayDescMarshaler.CleanUpNativeData2(ref arrayDesc);
		}
		else
		{
			_database.IBClient.isc_array_get_slice(
			_statusVector,
			ref dbHandle,
			ref trHandle,
			ref _handle,
	...
	public override void PutSlice(Array sourceArray, int sliceLength)
	{
		...
		if (_database.IBClient.IBClientVersion >= 7)
		{
			_database.IBClient.isc_array_put_slice2(
			_statusVector,
			ref dbHandle,
			ref trHandle,
			ref _handle,
		...
		{ 
			_database.IBClient.isc_array_put_slice(
			_statusVector,
			ref dbHandle,
			ref trHandle,
			ref _handle,
			...
			using (var writer = new BinaryWriter(ms))
			{
				...
				foreach (var source in sourceArray)
				{
					switch (dbType)
					{
						...
						case DbDataType.Decimal:
							{
								// Dialect 1 (IscCodes.SQL_DOUBLE) will be a double not a decimal
								var numeric = type != IscCodes.SQL_DOUBLE ? TypeEncoder.EncodeDecimal((decimal)source, desc.Scale, type) : Convert.ToDecimal((double)source);
