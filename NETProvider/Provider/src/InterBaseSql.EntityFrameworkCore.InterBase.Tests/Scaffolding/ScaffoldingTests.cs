...
namespace InterBaseSql.EntityFrameworkCore.InterBase.Tests.Scaffolding;
...
public class ScaffoldingTests : EntityFrameworkCoreTestsBase
{
	...
	public async Task ReadsCorrectFieldType(string dataType)
	{
		...
		try
		{
			commandTable.CommandText = $"drop table {tableName}";
			await commandTable.ExecuteNonQueryAsync();
		}
		catch { };
	...
	{
		return new IBDatabaseModelFactory();
	}
}